...
def ensure_answers_binding(text: str) -> str:
    ...
    if 'ANSWERS' not in text:
        return text
    anchor = re.search(r'/\* ---------- derived stats ---------- \*/', text)
    if anchor:
        return text[:anchor.start()] + 'const ANSWERS = AnswerOSData.getAnswers();\n' + text[anchor.start():]
...
def patch_dashboard(text: str) -> str:
    ...
    text = text.replace(
        "let calYear = 2026, calMonth = 7; // August 2026, matches most recent real data",
        "const latestAnswerDate = ANSWERS.length ? new Date(ANSWERS[0].date) : AnswerOSData.today();\nlet calYear = latestAnswerDate.getFullYear(), calMonth = latestAnswerDate.getMonth();",
    )
    text = text.replace(
        "const now = new Date('2026-08-16');",
        "const now = AnswerOSData.today();",
    )
    text = text.replace(
        "const now = new Date('2026-08-16');",
        "const now = AnswerOSData.today();",
